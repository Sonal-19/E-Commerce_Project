    ...
    const onSubmit = async (values) => {
        try {
          const response = await axios.post(
            'http://localhost:3059/users/api/addAdmin', values
          );
          console.log('Add adnin Successful:', response.data);
    
          if (response.data.success) {
            window.alert('Signup Successful!');
            navigate('/adminaccount');
          } else {
            console.log('Add Admin failed. Please check your information.');
          }
        } catch (error) {
          console.error('Error', error);
          console.log('An error occurred while add admin. Please try again later.');
        }
    };
